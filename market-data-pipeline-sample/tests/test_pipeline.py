...
class MarketDataPipelineTests(unittest.TestCase):
    def test_synthetic_fixture_accepts_three_and_quarantines_three(self) -> None:
    ...
    def test_outputs_are_deterministic_and_parquet_matches_jsonl(self) -> None:
        result = run_pipeline(input_csv=INPUT, aliases_path=ALIASES)
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            first_table = pq.read_table(first["normalized_parquet"])
            second_table = pq.read_table(second["normalized_parquet"])
            self.assertEqual(first_table.to_pylist(), second_table.to_pylist())
            self.assertEqual(first_table.num_rows, 3)
