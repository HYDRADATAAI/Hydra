...
OLD=ROOT/"constraint-replay"/"corpus"/"HYDRA_CONSTRAINT_CLASSIFIED_GOLD_UNCALIBRATED_BATCH009_20260926.jsonl"
NEW=ROOT/"constraint-replay"/"corpus"/"HYDRA_CONSTRAINT_CLASSIFIED_GOLD_UNCALIBRATED_BATCH010_20260926.jsonl"
ENRICH=ROOT/"constraint-replay"/"corpus"/"HYDRA_CONSTRAINT_REPLAY_OUTCOME_ENRICHMENT_BATCH010_20260926.json"
CONF=ROOT/"constraint-replay"/"corpus"/"HYDRA_CONSTRAINT_REPLAY_CONFIDENCE_ADMISSIBILITY_BATCH010_20260926.json"
MAP=ROOT/"constraint-replay"/"corpus"/"HYDRA_CONSTRAINT_REPLAY_OUTCOME_MAPPING_BATCH010_20260926.json"
AUDIT=ROOT/"constraint-replay"/"corpus"/"HYDRA_CONSTRAINT_REPLAY_PROMOTION_AUDIT_BATCH010_20260926.json"
...
class ClassifiedGoldBatch010ExpansionTests(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
    ...
    def test_expands_from_six_to_nine_without_rewriting_predecessors(self):
        self.assertEqual(6,len(self.old))
        self.assertEqual(9,len(self.new))
    ...
    def test_new_case_set_is_exact(self):
        ...
        self.assertEqual(
            {
                "us-china-section301-multi-stage-trade-dispute-2018",
    ...
    def test_new_mappings_are_partial_realization(self):
    ...
    def test_batch10_source_pins_match_checked_out_bytes(self):
        new_ids={r.case_id for r in self.new}-{r.case_id for r in self.old}
    ...
    def test_promotion_summary_is_nine_classified_zero_calibrated(self):
        summary=summarize_promotion(self.audits)
        self.assertEqual(22,summary["case_count"])
        self.assertEqual(9,summary["classified_gold_uncalibrated_count"])
        self.assertEqual(9,summary["outcome_class_supported_count"])
        self.assertEqual(9,summary["independent_hypothesis_present_count"])
    ...
    def test_nine_case_summary_remains_uncalibrated(self):
        summary=summarize_classified_gold(self.new)
        self.assertEqual(9,summary["case_count"])
        self.assertEqual({"PARTIAL_REALIZATION":9},summary["outcome_class_counts"])
