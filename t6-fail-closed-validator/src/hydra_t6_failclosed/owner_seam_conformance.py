...
def validate_owner_seams(
    *,
    ...
    gates = batch14_strict_gate.get("gates", {})
    if gates.get("CONSTRAINT_FORMATION", {}).get("strict_gate_result") != "FAIL":
        _fail("Batch014 strict constraint-formation gate is not fail-closed")
    if gates.get("BENEFICIARY_QUALIFICATION", {}).get("strict_gate_result") != "FAIL":
        _fail("Batch014 strict beneficiary gate is not fail-closed")
    if batch14_strict_gate.get("overall_result") != "BLOCKED":
